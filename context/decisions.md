...
| 4 | 13 Jan | Use real data for demos, not synthetic | Ben argued CSMs would fixate on errors in fake data rather than evaluating the system. Painful but forced early data quality confrontation. | Ben Brookes |
...
| 22 | 23 Jan | CSMs take accountability for data updates but involve full account team | CSMs set up 20-minute meetings with solution architects, implementation leads, and PMs to fill out data together. CSMs own accountability, not sole execution. | Ben Brookes, Natalia |
...
| 26 | 9 Feb | Holistic data fix, not incremental patches | Richard: "If we don't tackle it holistically, all that's going to happen is every week there's a new schema change, a new data fix." Kathryn Palkovics to sense-check complex accounts. | Richard |
...
| 37 | 6 Mar | Gainsight architecture alignment meeting planned | Meeting with Gainsight team (Tina Palumbo, Nadim, Rajesh) scheduled for next Thursday. Kathryn Palkovics offered governance help. Also pushing for Salesforce programmatic access. | Azmain, Kathryn Palkovics |
| 38 | 10 Mar | Idris given dedicated time for AI programme work | Ben Brookes approved. Idris to focus on TSR automation and become the AI champion back to the risk advisory team. | Ben Brookes, Richard |
...
| 42 | 9 Mar | Date input min set to 2022 across all CLARA date fields | Someone entered a 1999 date breaking the migration burndown chart. Ben Brookes asked for limits. Max also defined explicitly. | Azmain, Chris |
...
| 49 | 11 Mar | Salesforce integration one-way only (read from Salesforce, no write back) | Azmain: pushing data back is a huge can of worms given unknown SF data structure. Kathryn Palkovics agreed. Focus on pulling cases and case feed. | Richard, Kathryn Palkovics, Azmain |
| 50 | 11 Mar | Salesforce key data objects: Cases and Case Feed only (phase 1) | Success criteria and accounts not needed from Salesforce — those go to Gainsight. Four consumers: Bernard (sentiment), Courtney (HD), Kevin (tickets), CLARA (blockers). | Richard, Kathryn Palkovics |
...
| 54 | 11 Mar | Wednesday meeting next week to publicly launch App Factory | Richard wants Ben Brookes to present the MCP server to senior stakeholders. Timing deliberate — ISLTR in London. | Richard |
...
| 57 | 12 Mar | Gainsight-Clara integration starts with POC, charter-first approach | Ben Brookes insisted on formal charter before any development. Team strategy: demanding business requirements that slow integration without refusing. | Ben Brookes, Azmain |
| 58 | 12 Mar | Clara remains the UI and reporting layer; Gainsight used as data store; App Factory handles integration | Team consensus: even if Gainsight is source of truth for CSM data, Clara stays as the cross-functional UI | Ben Brookes, BenVH, Azmain, Richard |
...
| 61 | 12 Mar | Ben Brookes restricted to sandbox for CLARA changes | Can build and break in sandbox freely; must not push to user-facing production outside release cycle | Natalia Plant, Azmain |
...
| 64 | 12 Mar | Use case completion check: software status must be in production first | Blocking check, not automation — forces CSMs to manually update software status before closing use case | Azmain, Kathryn Palkovics, Natalia Plant |
...
| 67 | 12 Mar | Kathryn Palkovics (Catherine) situation to be raised with Natalia Orzechowska and Diya separately | Diana to discuss with Natalia; Richard to raise with Diya in London. Tactical approach: define AI enablement as separate from digital enablement. | Diana, Richard, Azmain |
...
| 73 | 14 Mar | CLARA-Gainsight Integration Charter V1 created with formal requirements approach | Azmain created charter; team uses it to control integration pace. Ben Brookes demanded SLAs: cannot slow migration, split CSMs, or block adoption squads. | Azmain, Ben Brookes |
| 74 | 14 Mar | Phase 0 structured CLARA feature review required before Gainsight integration proceeds | Richard: "Nobody from our side was involved in the initial assessment." Demands review with Gainsight team present + immediate RACI. | Richard |
| 75 | 16 Mar | Graduate allocation confirmed: Alvin (London) + Sam (New York) for CLARA, separate from Nikhil's Elliot replacement | Richard clarified to Emma Jaggs that Azmain's 2-grad request is separate from Nikhil's replacement need. Two different requirements. | Richard, Azmain |
| 76 | 16 Mar | Build in Five stakeholder cascade to be organized — MPS, Sales, Product | Similar to App Factory share-out approach. Samuel Gibson added to group chat as first external stakeholder engagement. | Richard, Azmain |
