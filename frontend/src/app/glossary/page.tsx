...
export default function GlossaryPage() {
  ...
  if (loading) return (<div className="space-y-6"><h2 className="text-2xl font-bold text-gray-900">Glossary</h2><p className="text-sm text-gray-500">Loading glossary...</p></div>);
  if (error) return (<div className="space-y-6"><h2 className="text-2xl font-bold text-gray-900">Glossary</h2><div className="rounded-lg border border-red-200 bg-red-50 p-6"><p className="text-sm text-red-700">{error}</p></div></div>);
  if (!glossary || Object.keys(glossary).length === 0) return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-900">Glossary</h2>
        <button onClick={openCreate} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 transition-colors">+ New Term</button>
      </div>
      <div className="text-center py-12"><p className="text-sm text-gray-500">No glossary entries yet.</p></div>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 items-center">
        <input type="text" placeholder="Filter terms..." value={filter} onChange={(e) => setFilter(e.target.value)}
          className="border rounded px-3 py-2 text-sm text-gray-700 bg-white w-72 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" />
        <span className="text-sm text-gray-500">{totalEntries} term{totalEntries !== 1 ? "s" : ""}{filter.trim() ? " matching" : ""}</span>
      </div>

      {categories.length === 0 ? (
        <div className="text-center py-12"><p className="text-sm text-gray-500">No terms match &ldquo;{filter.trim()}&rdquo;</p></div>
      ) : categories.map((category, idx) => (
        <div key={category} className={idx > 0 ? "border-t border-gray-200 mt-8 pt-8" : ""}>
            ...
            {(filteredGlossary?.[category] || []).map((entry: GlossaryEntrySchema) => (
              <div key={entry.id} className="px-6 py-4 group relative">
                ...
                <div className="flex items-start gap-2">
                  ...
                  {entry.aliases.length > 0 && <span className="text-xs text-gray-400 mt-0.5">({entry.aliases.join(", ")})</span>}
                </div>
                <p className="text-sm text-gray-700 mt-1">{entry.definition}</p>
              </div>
            ))}
          </div>
        </div>
