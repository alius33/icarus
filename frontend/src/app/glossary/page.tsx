...
export default function GlossaryPage() {
  ...
  if (loading) return (<div className="space-y-6"><h2 className="text-2xl font-bold text-gray-900">Glossary</h2><p className="text-base text-gray-500">Loading glossary...</p></div>);
  ...
  return (
    <div className="space-y-6">
      ...
      {categories.length === 0 ? (
        <div className="text-center py-12"><p className="text-base text-gray-500">No terms match &ldquo;{filter.trim()}&rdquo;</p></div>
            ...
            {(filteredGlossary?.[category] || []).map((entry: GlossaryEntrySchema) => (
              <div key={entry.id} className="px-6 py-4 group relative">
                ...
                <div className="flex items-start gap-2">
                  <span className="text-base font-bold text-gray-900">{entry.term}</span>
                  {entry.aliases.length > 0 && <span className="text-sm text-gray-400 mt-0.5">({entry.aliases.join(", ")})</span>}
                </div>
                <p className="text-base text-gray-700 mt-1">{entry.definition}</p>
              </div>
            ))}
          </div>
        </div>
