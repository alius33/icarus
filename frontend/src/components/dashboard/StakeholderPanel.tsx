...
function TrendLabel({ trend }: { trend: "rising" | "stable" | "declining" | "silent" }) {
  const colors: Record<string, string> = {
    ...
    stable: "text-forest-400",
    declining: "text-amber-600",
    silent: "text-red-500",
  };
  return (
    <span className={`text-[10px] font-medium ${colors[trend] || "text-forest-300"}`}>
      {trend}
    </span>
  );
}
...
function EngagementBar({
  ...
  return (
    <div className="flex items-center gap-1.5 w-24">
      ...
      <span className="text-[10px] font-medium text-forest-400 tabular-nums w-4 text-right">
        {recent}
      </span>
    </div>
  );
...
export default function StakeholderPanel({ items, onItemClick }: Props) {
  ...
  return (
    <div>
      <h4 className="text-sm font-semibold text-forest-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
        <Users className="h-3.5 w-3.5" />
        Stakeholder Engagement
      </h4>
      {items.length === 0 ? (
        <p className="text-base text-forest-400">No tier 1-2 stakeholder data.</p>
      ) : (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-base font-medium text-forest-950 group-hover:text-forest-600 truncate">
                    {s.name}
                  </span>
                  <span className="inline-flex items-center rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-medium text-forest-600">
                    T{s.tier}
                  </span>
                </div>
                {s.role && (
                  <p className="text-[10px] text-forest-300 truncate">{s.role}</p>
                )}
              </div>
              <EngagementBar
