...
function DroppableColumn({
  ...
  const cfg = DECISION_STATUS_CONFIG[column.status as DecisionStatus] ?? { label: column.label, color: "text-forest-500", bgColor: "bg-forest-100" };
      ...
      {!collapsed && (
        <div className="flex-1 p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-240px)]">
          ...
          {column.decisions.length === 0 && (
            <p className="text-sm text-forest-300 text-center py-4">No decisions</p>
          )}
        </div>
      )}
    </div>
