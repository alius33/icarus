  ...
  return (
    <div className="rounded-lg border border-forest-200 dark:border-forest-700 bg-white dark:bg-forest-800 p-4">
      <div className="flex items-center justify-between text-xs text-forest-300 mb-2">
        <span>23 Feb</span>
        <span className="font-medium text-forest-950 dark:text-white">Week {Math.min(currentWeek, TOTAL_WEEKS)} of {TOTAL_WEEKS}</span>
        <span>15 May</span>
      </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isPast ? "bg-green-600" : isCurrent ? "bg-forest-500" : "bg-gray-200 dark:bg-forest-800"
              )}
              title={`Week ${weekNum}`}
            />
          );
      ...
      <div className="flex items-center gap-4 mt-2 text-[10px] text-forest-400">
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-green-600" /> Completed</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-forest-500" /> Current</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-gray-200 dark:bg-forest-800" /> Remaining</span>
      </div>
    </div>
  );
}
...
function MilestoneStatusIcon({ status }: { status: string }) {
  switch (status) {
    ...
    case "IN_PROGRESS": return <Clock className="h-3.5 w-3.5 text-forest-500" />;
    case "BLOCKED": return <AlertCircle className="h-3.5 w-3.5 text-red-500" />;
    default: return <Minus className="h-3.5 w-3.5 text-forest-400" />;
  }
}
...
function MilestoneRow({ milestone }: { milestone: DeliverableMilestone }) {
  ...
  return (
    <div className="flex items-start gap-3 py-2 px-3 rounded hover:bg-forest-100 dark:hover:bg-forest-700/50">
      <MilestoneStatusIcon status={milestone.status} />
      <div className="flex-1 min-w-0">
        <p className="text-sm text-forest-600 dark:text-forest-200">{milestone.title}</p>
        {milestone.evidence && (
          <p className="text-xs text-forest-400 mt-0.5">{milestone.evidence}</p>
        )}
      </div>
      <div className="flex items-center gap-2 flex-shrink-0">
        {milestone.target_week && (
          <span className="text-[10px] text-forest-400">Wk {milestone.target_week}</span>
        )}
...
function DeliverableRow({ deliverable }: { deliverable: ProgrammeDeliverable }) {
  ...
  return (
    <div className="border-t border-forest-200 dark:border-forest-700/50 first:border-t-0">
      <button
        onClick={() => setExpanded(!expanded)}
        className="flex items-center gap-3 w-full px-4 py-3 text-left hover:bg-gray-800/30 transition-colors"
      >
        {expanded ? <ChevronDown className="h-4 w-4 text-forest-300" /> : <ChevronRight className="h-4 w-4 text-forest-300" />}
        <span className={cn("h-2 w-2 rounded-full flex-shrink-0", ragCfg.dotColor)} />
        <span className="text-sm text-forest-600 dark:text-forest-200 flex-1 truncate">{deliverable.title}</span>
        <span className="text-xs text-forest-400">{completedMs}/{totalMs}</span>
        <div className="w-20 h-1.5 rounded-full bg-gray-200 dark:bg-forest-800 flex-shrink-0">
          <div
...
function PillarSection({ deliverables }: { deliverables: ProgrammeDeliverable[] }) {
  ...
  return (
    <div className="rounded-lg border border-forest-200 dark:border-forest-700 bg-white dark:bg-forest-800 overflow-hidden">
      <button
        onClick={() => setCollapsed(!collapsed)}
        className="flex items-center gap-3 w-full px-4 py-3 hover:bg-forest-100 dark:hover:bg-gray-750 transition-colors"
      >
        ...
        <h3 className="text-sm font-semibold text-forest-950 dark:text-white flex-1 text-left">{pillarName}</h3>
        <span className={cn("text-xs px-2 py-0.5 rounded-full", ragCfg.bgColor, ragCfg.color)}>
          {ragCfg.label}
        </span>
        <div className="w-24 h-2 rounded-full bg-gray-200 dark:bg-forest-800">
          <div
            className={cn("h-2 rounded-full", rag === "RED" ? "bg-red-500" : rag === "AMBER" ? "bg-amber-500" : "bg-green-500")}
            style={{ width: `${avgProgress}%` }}
          />
        </div>
        <span className="text-xs text-forest-400 w-10 text-right">{avgProgress}%</span>
      </button>
      {!collapsed && (
        <div>
          {deliverables.map((d) => (
...
export default function DeliverablesTab() {
  ...
  if (!deliverables) {
    return <div className="text-forest-300">Loading deliverables...</div>;
  }
