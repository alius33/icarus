  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-800 p-4">
      <div className="flex items-center justify-between text-xs text-gray-400 mb-2">
        <span>23 Feb</span>
        <span className="font-medium text-white">Week {Math.min(currentWeek, TOTAL_WEEKS)} of {TOTAL_WEEKS}</span>
        <span>15 May</span>
      </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isPast ? "bg-green-600" : isCurrent ? "bg-blue-500" : "bg-gray-700"
              )}
              title={`Week ${weekNum}`}
            />
          );
      ...
      <div className="flex items-center gap-4 mt-2 text-[10px] text-gray-500">
        ...
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-gray-700" /> Remaining</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex items-start gap-3 py-2 px-3 rounded hover:bg-gray-800/50">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-gray-200">{milestone.title}</p>
        {milestone.evidence && (
  ...
  return (
    <div className="border-t border-gray-700/50 first:border-t-0">
      ...
      >
        ...
        <span className="text-sm text-gray-200 flex-1 truncate">{deliverable.title}</span>
        <span className="text-xs text-gray-500">{completedMs}/{totalMs}</span>
        <div className="w-20 h-1.5 rounded-full bg-gray-700 flex-shrink-0">
          <div
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-800 overflow-hidden">
      <button
        ...
        className="flex items-center gap-3 w-full px-4 py-3 hover:bg-gray-750 transition-colors"
      >
        ...
        <h3 className="text-sm font-semibold text-white flex-1 text-left">{pillarName}</h3>
